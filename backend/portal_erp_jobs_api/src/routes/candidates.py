...
def get_public_candidates():
    """
    Listar candidatos com currículo público (sem autenticação)
    """
    try:
        # Parâmetros de busca
        query = request.args.get('q', '')
        ...
        # Construir query - apenas candidatos com currículo público
        candidates_query = Candidate.query.filter(Candidate.curriculo_publico == True)
        ...
        # Buscar dados completos para cada candidato
        candidates_data = []
        for candidate in pagination.items:
            ...
            candidate_dict = {
                'id': candidate.id,
                'name': f"{candidate.first_name or ''} {candidate.last_name or ''}".strip() or 'Candidato',
                'title': candidate.current_title or 'Profissional',
                'location': candidate.city or 'Brasil',
                'cidade': candidate.city,
                'estado': candidate.state,
                'email': user.email if user else None,
                'linkedin': candidate.linkedin_url,
                'github': candidate.github_url,
                'pretensao_salarial': candidate.expected_salary,
                'disponibilidade': 'Imediata' if candidate.is_actively_looking else 'A combinar',
                'technologies': [{'name': skill.skill_name, 'level': skill.proficiency_level} for skill in skills]
            }
        ...
        print(f"Error getting public candidates: {e}")
        return jsonify({'error': str(e)}), 500
